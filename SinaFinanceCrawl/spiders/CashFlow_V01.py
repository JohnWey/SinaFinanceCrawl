...
path_cashflow = conf.get('Stock', 'path_outputfile').replace('?filename','CashFlow')
urls = conf.get('Stock','url_link').replace('?type','vFD_CashFlow')

stockcode_list = [ x.replace("\'","") for x in stockcodes.split(',') ]

#stockcode_list = ['000001']
reportDT_list = ['2016']
sys.stdout=open(path_cashflow+reportDT_list[0]+'.txt','w')

class CashFlow(Spider):
    name = "cfcrawl"
    allowed_domains = []
    ...
    def parse(self, response):
        ...
        if v_cnt == "94.0" or v_cnt == "151.0":
            ...
            for v_col_index in range(2,i+1):
                ...
                print "\'"+stockcode+"\'"+",\'"+v_itemSet[0]+'\','+','.join(v_itemSet_fmt[1:]).replace("--","0")
        else:
            print "=======================NONE============================="
