...
path_profit = conf.get('Stock', 'path_outputfile').replace('?filename','Profit')
urls = conf.get('Stock','url_link').replace('?type','vFD_ProfitStatement')

stockcode_list = [ x.replace("\'","") for x in stockcodes.split(',') ]

#stockcode_list = ['000748']
reportDT_list = ['2007']
...
class Profit(Spider):
    ...
    def parse(self, response):
        ...
        sys.stdout = open(path_profit + reportDT_list[0] + '.txt', 'a')
        ...
        v_cnt = sel.xpath('count(//table[@id="ProfitStatementNewTable0"]/tbody/tr/td[1])').extract()[0]
        v_itemSet = []
        if v_cnt == "46.0" or v_cnt == "50.0":
            #Get the Report date count, if the Report date is current year, it's different with previous year data.
            v_reportdate = sel.xpath('count(//table[@id="ProfitStatementNewTable0"]/tbody/tr[1]/td)').extract()[0]
            i = int(v_reportdate[0]) #Get the Report date count
            #Get the Report data
            for v_col_index in range(2,i+1):
                v_url = "//table[@id='ProfitStatementNewTable0']/tbody/tr/td["+str(v_col_index)+"]/text()"
                v_itemSet = sel.xpath(v_url).extract()
                v_itemSet_fmt = [ x.replace(',','') for x in v_itemSet ]

                print "\'"+stockcode+"\'"+",\'"+v_itemSet[0]+'\','+','.join(v_itemSet_fmt[1:]).replace("--","0").replace("（元）","")
